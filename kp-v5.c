...
#include <stdlib.h>
#include <time.h>
double indicator(int ts, float mp, int minp);
...
double indicator(int ts, float mp, int minp) {
	double tsf = (ts + 273.15) / 647.14;
	double x = 1 - tsf;
	double drob = (-7.85823 + 1.83991 * pow(x, 1.5) - 11.7811 * pow(x, 3) + 22.6705 * pow(x, 3.5) - 15.9393 * pow(x, 4) + 1.77516 * pow(x, 7.5)) / ts;
	double ps = 22.064 * exp(drob);
	double razl = fabs(ps - mp) / fabs(mp) * 100 * (minp / 100.0);
	return razl;
}
